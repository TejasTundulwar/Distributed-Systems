...
//add here random value into nearest neighbour data structure
void recv_sharing(uint8_t *payload, uint8_t distance)
{
    ...
    mydata->nearest_neighbors[i].random_value = payload[RANDOM_VALUE]; //random value
    ...
    if(payload[ID]==mydata->my_left){
        mydata->my_left_color=payload[COLOR];
    }
    if(payload[ID]==mydata->my_right){
        mydata->my_right_color=payload[COLOR];
    }
}
...
void recv_color(uint8_t *payload)
{
    if(mydata->my_color > 5 ) { //|| payload[COLOR] > 5){
        ...
        if(payload[ID]==mydata->my_right && mydata->my_left!=mydata->my_right)
        {
             mydata->my_color = getRoundcolor(mydata->my_color,payload[COLOR]);
            //mydata->my_right_color=payload[COLOR];
            #ifdef SIMULATOR
           printf("(Not in range) MyId:%d MyColor:%d\n",mydata->my_id,mydata->my_color);
            #endif
    ...
    if(payload[COLOR]==mydata->my_color && payload[ID]==mydata->my_left)
    {
        mydata->red=0;
        mydata->blue=0;
        mydata->green=0;
    ...
    if((mydata->my_color < 6 && mydata->my_color >2) && (mydata->my_left_color < 6) && (mydata->my_right_color < 6)) {
        
        if(mydata->my_color > payload[COLOR]){
           //select first available color
            
                mydata->my_color=0;
                if(mydata->my_left_color==0 || mydata->my_right_color==0){
                    mydata->my_color=mydata->my_color+1;
                }
                if(mydata->my_left_color==1 || mydata->my_right_color==1){
                    mydata->my_color=mydata->my_color+1;
                }
            
            
           /*  while(mydata->my_color==mydata->my_left_color || mydata->my_color > 2){
                mydata->my_color=(mydata->my_left_color+1)%3;
                printf("(6-3) MyId:%d MyColor:%d MyleftColor:%d MyrightColor:%d\n ",mydata->my_id,mydata->my_color,mydata->my_left_color,mydata->my_right_color);
            }while ends here*/
            #ifdef SIMULATOR
            printf("(6-3) MyId:%d MyColor:%d\n",mydata->my_id,mydata->my_color);
            #endif
            }
    }
    if(mydata->my_color < 3) {
        mydata->red=0;
        mydata->blue=0;
        mydata->green=0;
        if(mydata->my_color==0){
            mydata->red=1;
        } else if (mydata->my_color==1) {
            mydata->blue=1;
        } else {
            mydata->green=1;
        }
...
void prepare_message(uint8_t m, uint8_t receiver)
{
    ...
    mydata->msg.data[RANDOM_VALUE]=mydata->my_random_value;//seting random value
   // mydata->msg.data[];
...
void wakeup(){
	if(mydata->now >= mydata->next_wakeup &&  mydata->state == NOMIS){
            for(uint8_t i=0;i<=mydata->num_neighbors;i++){
                if( mydata->nearest_neighbors[i].state == MIS && mydata->nearest_neighbors[i].random_value < mydata->my_random_value){
                    mydata->state=PASSIVE;
                    mydata->red=1;
                    mydata->green=0;
                    mydata->blue=0;
		    #ifdef SIMULATOR
			printf("ID:%d entered PASSIVE\n", mydata->my_id);
		    #endif
                    break;
            	}
             }
    	     mydata->next_wakeup = mydata->now + WAKEUPTIME;
        }
}

void mis(){
    uint8_t flag=0;
    if(mydata->state==NOMIS && mydata->now >= mydata->next_mis){
        for (uint8_t i=0; i<mydata->num_neighbors; i++)
        {
            if(mydata->my_random_value > mydata->nearest_neighbors[i].random_value)
            {
                flag=1;
                break;
            }
        ...
        if(flag == 0){
            ...
            mydata->blue=0;
	    #ifdef SIMULATOR
		printf("ID:%d entered MIS\n", mydata->my_id);
	    #endif
        }
        mydata->next_mis = mydata->now + MISTIME;
    }
    
}

void stabilization(){
    if(mydata->now >= mydata->next_stabilization &&  mydata->state == MIS){
	for(uint8_t i=0;i<=mydata->num_neighbors;i++){
            if(mydata->nearest_neighbors[i].state == MIS){
                #ifdef SIMULATOR
                    printf("ID:%d entered stabilization\n", mydata->my_id);
                #endif
                mydata->state=NOMIS;
                mydata->red=0;
                mydata->green=0;
    ...
    mydata->next_stabilization = mydata->now + STABILIZATIONTIME;
    }
...
void setup() {
    ...
    mydata->my_random_value=rand_soft();//new for mis
    mydata->state = COOPERATIVE;
